...
export function drawTankShadow(ctx: CanvasRenderingContext2D, x: number, y: number) {
  ...
  const shadowColor = PALETTE.BLACK_PURE + "80";
...
export function drawTankBody(ctx: CanvasRenderingContext2D, params: TankDrawParams) {
  ...
  const selfShadowColor = PALETTE.BLACK_PURE + "59";
  ...
  if (params.hasShield) {
    ...
    ctx.lineWidth = 2;
    ctx.fillStyle = PALETTE.BLUE_INFO + "40";
    ctx.beginPath();
...
export function drawTankHealthBar(
  ...
  ctx.fillStyle = PALETTE.BLACK_PURE + "80";
  ctx.beginPath();
