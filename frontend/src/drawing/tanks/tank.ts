...
export function drawTankShadow(ctx: CanvasRenderingContext2D, x: number, y: number) {
  ...
  const shadowColor = PALETTE.BLACK_PURE_50;
...
export function drawTankBody(ctx: CanvasRenderingContext2D, params: TankDrawParams) {
  ...
  const selfShadowColor = PALETTE.BLACK_PURE_35;
  ...
  if (params.hasShield) {
    ...
    ctx.lineWidth = 2;
    ctx.fillStyle = "PALETTE.BLUE_INFO_25";
    ctx.beginPath();
...
export function drawTankHealthBar(
  ...
  ctx.fillStyle = PALETTE.BLACK_PURE_50;
  ctx.beginPath();
