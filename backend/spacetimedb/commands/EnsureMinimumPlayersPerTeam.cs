...
{
    public static class EnsureMinimumPlayersPerTeam
    {
        public static void Call(ReducerContext ctx, string gameId)
        {
            ...
            if (alliance0Players < minPlayers)
            {
                ...
                for (int i = 0; i < botsNeeded; i++)
                {
                    ...
                    var (spawnX, spawnY) = FindSpawnPosition.Call(ctx, game.Value, 0, ctx.Rng);
            ...
            if (alliance1Players < minPlayers)
            {
                ...
                for (int i = 0; i < botsNeeded; i++)
                {
                    ...
                    var (spawnX, spawnY) = FindSpawnPosition.Call(ctx, game.Value, 1, ctx.Rng);
                    var botName = GenerateBotName.Call(ctx, gameId);
                    var (botTank, botTransform) = BuildTank(
                        ctx: ctx,
                        gameId: gameId,
